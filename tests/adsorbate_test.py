...
def test_adsorbate_initialization_h2o_ase(dummy_water):
    ...
    assert isinstance(adsorbate.structure, list)
    assert isinstance(adsorbate.structure[0], Atoms)
    assert adsorbate.structure[0] == molecule("H2O")
...
def test_adsorbate_initialization_co2_ase(dummy_co2):
    ...
    assert isinstance(adsorbate.structure, list)
    assert isinstance(adsorbate.structure[0], Atoms)
    assert adsorbate.structure[0] == molecule("CO2")
...
def test_adsorbate_initialization_h2o_file(h2o_path):
    ...
    assert isinstance(adsorbate.structure, list)
    assert isinstance(adsorbate.structure[0], Atoms)
    assert adsorbate.structure[0] == read(str(h2o_path))
    assert adsorbate.molar_mass == pytest.approx(18.015, 0.1)
...
def test_adsorbate_initialization_co2_file(co2_path):
    ...
    assert isinstance(adsorbate.structure, list)
    assert isinstance(adsorbate.structure[0], Atoms)
    assert adsorbate.structure[0] == read(str(co2_path))
    assert adsorbate.molar_mass == pytest.approx(44.01, 0.1)
...
def test_eos_dict_without_structure_raises_error(dummy_water):
    """Test that EOS dict fails if there is no structure to get the molar mass from."""
    adsorbate = Adsorbate("H2O", structure=dummy_water)
    adsorbate.structure = None  # Forcefully bypass init to test setter
    ...
    with pytest.raises(ValueError, match="Structure must be set before setting EOS parameters"):
        adsorbate.eos = eos_params
